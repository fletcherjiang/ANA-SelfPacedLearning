...
def train(optimizer, train_loader, weights, args, resnet50, loss_function, device):
    print('train:')
    t = time.time()
    resnet50.train()
    train_loss = 0. # sum of train loss up to current batch
...
#     global current_training_iteration
    ...
    for batch_num, (data, target, index) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training Progress", unit="batch"):
        if target.sum() == 0:
            continue
        #print('loader', time.time() - t)
        weight = weights[index]
        ...
        weight = weight.index_select(0, torch.nonzero(target.sum(dim=1)).view(-1))
        
        target = target * F.relu(weight)
        target = (target - target.min(dim = 1, keepdim = True)[0]) / (target.max(dim = 1, keepdim = True)[0] - target.min(dim = 1, keepdim = True)[0])
        target[torch.isnan(target)] = 0
        target = target.detach().bernoulli().float()
        #print('filter', time.time() - t)
...
def test(resnet50,args,device):
    ...
    dataframe = pd.read_csv('data_8_class_single.csv')
    dataframe = pd.read_csv('data_8_class_single.csv').head(10)
...
def test_single(resnet50,args,device):
    print('val single:')
    t = time.time()
    resnet50.eval()
    ...
    dataframe = dataframe[dataframe['Split'] ==  'val']
    dataframe = dataframe[np.array(list(map(len, dataframe['TARGET']))) == 1] #Read data with a single label
    dataframe = dataframe.reset_index(drop = True)
    ...
    for i in tqdm(range(len(dataframe)), desc="Testing Progress", unit="sample"):
        ...
        #temp_images = get_small(temp_path)
        #temp_images = temp_images.cuda()
        #output, probabilities = inference(resnet50,temp_images)
        #analysis_prob.append(probabilities.detach().cpu().numpy())
    ...
    ##prediction
    temp = np.zeros((len(dataframe),8))
    for i in range(len(analysis_tar)):
        temp[i,:] = analysis_prob[i].max(dim=0)[0].detach().cpu().numpy()
    tt = temp>0.5
    
    ##计算真实标签
    ans = np.zeros((len(dataframe),8))
    for i in range(len(analysis_tar)):
        cur_ans = analysis_tar[i] 
        _ = np.zeros((1,8))
        for j in cur_ans:
    ...
    print(sm.classification_report(targets,prediction_labels))
    print('test single done', time.time() - t)
    return acc_0 ,acc_1 ,acc_2 ,acc_3 ,acc_4 ,acc_5 ,acc_6 ,acc_7 , acc_total,f1_mi,f1_ma, mAP
...
def test_multiple():
    ...
    print(sm.classification_report(targets,prediction_labels))
    print('test multiple done', time.time() - t)
    return acc_0 ,acc_1 ,acc_2 ,acc_3 ,acc_4 ,acc_5 ,acc_6 ,acc_7 , acc_total,f1_mi,f1_ma, mAP
...
def test_small(val_loader,resnet50,device):
    ...
    mAP = average_precision_score(targets, prob, average='macro')  # 'macro' 是计算每个类的平均精度
    acc_total = accuracy_score(targets, prediction_labels)
    from sklearn.metrics import f1_score
    f1_mi = f1_score(targets, prediction_labels, average='micro')
    f1_ma = f1_score(targets, prediction_labels, average='macro')
    # acc_total = sm.accuracy_score(targets,prediction_labels)
    # f1_mi = sm.f1_score(targets,prediction_labels,average='micro')
    # f1_ma = sm.f1_score(targets,prediction_labels,average='macro')
...
def get_train_dataset_single():
    ...
    train_dataset = get_traindata_single(root=DIR_TRAIN_IMAGES,
                                  annFile=path_csv, 
...
def get_train_dataloader(model, weights, targets, train_dataset, args):
    sw = (F.relu(weights) * targets).max(dim = 1)[0]
